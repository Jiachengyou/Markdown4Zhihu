...
首先transformer通过三个矩阵$$W^q,W^k,W^v$$将$$x^i$$分成$$q^i,k^i,v^i$$三个vector，分别对应query，key，value，其中query用于查找其他$$x^i$$的key，用于使他们彼此产生关系，而value用来表示他们各自含有的信息。

![image-20200701230254634](D:\program\Typora\img\BERT 可能是目前最强的NLP模型\image-20200701230254634.png)

在得到$$q^i,k^i,v^i$$之后，我们将$$q^i$$与其他的key点乘，得到$$\hat{\alpha}_{i,j}$$随后计算$$b^i=\sum_j\hat{\alpha}_{i,j}*v^j$$,这样我们就可以得到第一层的$$x^i$$对应的$$b^i$$ 。

上述过程是可以被平行化的，整体的公式如下：

$$Q = W^qX, K = W^kX, V = W^vX$$，得到query,key,value，对应图一

 $$B = Q^TK*V/\sqrt{d}$$,完成self-Attention，得到输出，对应图二
...
如上图，所谓multi-head,其实就是将$$q^i,k^i,v^i$$分成多个部分，得到多个$$b^i$$，最后将$$b^{i,j}$$根据所需维数拼接起来。
...
对每一个$$x^i$$，加入$$e^i$$（代表位置信息）,进行位置编码。

当然还有另一种做法，就是拼接，就是把代表位置的信息拼接在$$x^i$$后面。
